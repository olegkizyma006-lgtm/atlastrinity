...
class VectorStorage:
    ...
    def store(self, data: dict[str, Any] | list[dict[str, Any]]) -> StorageResult:
        ...
        try:
            ...
            logger.info(f"Persisted {record_count} records to ChromaDB collection '{self.collection_name}'")
            ...
            if results["ids"]:
                for i in range(len(results["ids"][0])):
                    formatted_results.append({
                        "id": results["ids"][0][i],
                        "score": 1.0 - results["distances"][0][i] if results["distances"] else 0, # Chroma returns distance
