...
class RecursiveEnricher:
    ...
    async def enrich_and_search(self, query: str, depth: int = 0) -> str:
        ...
        logger.info(f"Local confidence low/miss for '{query}'. Initiating external enrichment (Depth {depth})")
        ...
        for pkg in packages:
            ...
            if resources:
                ...
                ingest_result = await ingest_dataset(url, type="api", process_pipeline=["parse"]) # Assume API/Direct link
