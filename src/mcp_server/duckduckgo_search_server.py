...
def duckduckgo_search(query: str, max_results: int = 5, timeout_s: float = 10.0, step_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if not results:
             return {"success": False, "error": "Zero results found. DDG might be blocking or layout changed.", "query": query.strip()}
...
def business_registry_search(company_name: str, step_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return _execute_protocol_search("business", company_name, "DuckDuckGo (Optimized Registry Search)")


@server.tool()
def open_data_search(query: str, step_id: Optional[str] = None) -> Dict[str, Any]:
    """
