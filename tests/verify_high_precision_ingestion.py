...
async def verify_ingestion_pipeline():
    ...
    if clean_res.get("success") is True:
        ...
        table_name = clean_res["table_name"]
    ...
    if clean_res.get("success"):
        ...
        async with await db_manager.get_session() as session:
            ...
            raw_data_res = await session.execute(
                text(f"SELECT * FROM {table_name}")
            )
            raw_data = raw_data_res.fetchall()
            if len(raw_data) == 3:
                print(f"SUCCESS: Structured table '{table_name}' contains all {len(raw_data)} rows.")
            else:
                print(f"FAILURE: Structured table '{table_name}' has incorrect row count: {len(raw_data)}")
