...
async def test_v24_logic():
    ...
    data2 = {
        "contact_email": ["atlas@trinity.ai"],
        "role": ["Meta-Planner"]
    }
    ...
    await knowledge_graph.add_node("DATASET", f"dataset:{table_name}", {"columns": list(df.columns)})
    await knowledge_graph.add_node("DATASET", f"dataset:{table_name2}", {"columns": list(df2.columns)})
    ...
    await knowledge_graph.add_node("ENTITY", "entity:test_fact", {"value": "Trinity is Omniscient"}, namespace="task_123")
    
    # Mocking a promotion (we can just add another node in global for now)
    await knowledge_graph.add_node("ENTITY", "entity:test_fact", {"value": "Trinity is Omniscient"}, namespace="global")
