...
class DataValidator:
    ...
    def validate_data_completeness(self) -> dict[str, Any]:
        ...
        result = {"checks": [], "valid": True, "issues": []}
        ...
        for file_path in critical_files:
            path = Path(file_path)
            exists = path.exists()
            result["checks"].append({"file": file_path, "exists": exists})
    ...
    def generate_validation_report(self) -> dict[str, Any]:
        ...
        for db_result in db_results:
            report["results"].append({"type": "database_schema", **db_result})

        # 3. Tool schemas
        tool_schemas_result = self.validate_tool_schemas()
        report["results"].append({"type": "tool_schemas", **tool_schemas_result})

        # 4. MCP catalog
        mcp_catalog_result = self.validate_mcp_catalog()
        report["results"].append({"type": "mcp_catalog", **mcp_catalog_result})

        # 5. Data completeness
        completeness_result = self.validate_data_completeness()
        report["results"].append({"type": "data_completeness", **completeness_result})
