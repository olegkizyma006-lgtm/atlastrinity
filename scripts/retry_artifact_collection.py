...
async def run():
    ...
    notes_search = await mcp_manager.call_tool('notes','search_notes', {'tags':['step_424'], 'limit':5})
    structured = getattr(notes_search,'structuredContent', None) or getattr(notes_search, 'content', None)
    ...
    if note_id:
        ...
        elif hasattr(note,'content') and len(note.content)>0 and hasattr(note.content[0],'text'):
            import json
            try:
