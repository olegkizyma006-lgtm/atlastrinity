...
def test_data_validation():
    ...
    # Test valid data
    valid_data = {"name": "Test User", "age": 30, "city": "Test City", "score": 85.5}

    result = transformer.validate_data(valid_data, "test")
    print(
        f"Valid data validation: {'✓' if result.success else '✗'} {result.error if not result.success else ''}"
    )
...
def test_schema_compatibility():
    ...
    compatible_data = [{"name": "John", "age": 25, "city": "New York", "score": 85.5}]

    result = transformer.validate_schema_compatibility(compatible_data)
    print(
        f"Compatible schema check: {'✓' if result.success else '✗'} {result.data.get('message') if result.success else result.error}"
    )
...
def test_error_handling():
    ...
    print(
        f"Empty data: {'✓' if result.success else '✗'} {len(result.data) if result.success else result.error}"
    )
    print()
